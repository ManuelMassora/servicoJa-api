...
type Transacao struct {
	BaseModel
	IDUsuario     int64        `json:"usuario_id" gorm:"not null"`
...
type TransacaoRepo interface {
	Criar(ctx context.Context, transacao *Transacao) error
		ListarPorUsuario(ctx context.Context, idUsuario int64, filters map[string]interface{}, orderBy string, orderDir string, limit, offset int) ([]Transacao, error)
}
